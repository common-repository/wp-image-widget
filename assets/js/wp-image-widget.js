...
(function( window, $, _, Backbone, wp, undefined ) {
	...
	var Control, l10n,
		WPImageWidget = window.WPImageWidget,
		Attachment = wp.media.model.Attachment,
		finder = { view: {} },
		frames = [];

	// Link any localized strings.
	l10n = WPImageWidget.l10n = WPImageWidget.l10n || {};
	...
	Control = function( el, options ) {
		...
		selector = this.$el.data( 'target' ) || '.wp-image-widget-control-target';
	...
	_.extend( WPImageWidget, {
		...
		getControl: function( el ) {
			...
			$control = $( el ).closest( '.wp-image-widget-control' );
	...
	finder.view.Page = wp.Backbone.View.extend({
		events: {
			'click .wp-image-widget-find-posts-button' : 'openModal'
		},
		...
		openModal: function( e ) {
			var $target = $( e.target).closest( '.wp-image-widget-input-group' ).find( 'input[type="text"]' );
			e.preventDefault();
	...
	finder.view.Modal = wp.Backbone.View.extend({
		tagName: 'div',
		className: 'find-box',
		template: wp.template( 'wp-image-widget-modal' ),

		events : {
			'click .wp-image-widget-modal-search-button': 'findPosts',
			'keypress .wp-image-widget-modal-search-field': 'routeKey',
			'keyup .wp-image-widget-modal-search-field': 'routeKey',
			'click tr': 'select',
		...
		render: function() {
			...
			this.$field = this.$el.find( '.wp-image-widget-modal-search-field' );
			this.$overlay = $( '.ui-find-overlay' );
			this.$response = this.$el.find( '.wp-image-widget-modal-response' );
			this.$spinner = this.$el.find( '.wp-image-widget-modal-search .spinner' );
			return this;
		},
	...
	$( document ).ready(function() {
		...
		$body.on( 'click', '.wp-image-widget-control-choose, .wp-image-widget-form img', function( e ) {
			e.preventDefault();
			WPImageWidget.getControl( this ).frame().open();
		});

		// Update the image preview and size dropdown in a widget when an image is selected.
		$body.on( 'selectionChange.simpleimagewidget', '.wp-image-widget-control', function( e, selection ) {
			var $control = $( e.target ),
				$sizeField = $control.closest( '.wp-image-widget-form' ).find( 'select.image-size' ),
			...
			if ( $sizeField.length ) {
				WPImageWidget.updateSizeDropdownOptions( $sizeField, sizes );
			}
			...
			$control.find( 'img' ).remove().end()
				.prepend( image )
				.addClass( 'has-image' )
				.find( 'a.wp-image-widget-control-choose' ).removeClass( 'button-hero' );
		});

		// Wire up the toggle checkboxes in the screen options tab.
		$( '.wp-image-widget-field-toggle' ).on( 'click', function() {
			var $this = $( this ),
				field = $this.val(),
				$hiddenFields = $( '.wp-image-widget-field-toggle:not(:checked)' );

			$( '.wp-image-widget-field-' + field ).toggleClass( 'is-hidden', ! $this.prop( 'checked' ) );
			...
			$.ajax({
				...
				data: {
					...
					nonce: WPImageWidget.screenOptionsNonce
				},
				success: function( data ) {
					if ( 'nonce' in data ) {
						WPImageWidget.screenOptionsNonce = data.nonce;
					}
				}
			});
		});

		$body.on( 'focus', '.wp-image-widget-input-group-field', function() {
			$( this ).parent().addClass( 'is-focused' );
		}).on( 'blur', '.wp-image-widget-input-group-field', function() {
