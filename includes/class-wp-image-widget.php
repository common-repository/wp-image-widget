...
class WP_Image_Widget extends WP_Widget {
	...
	public function __construct( $id_base = false, $name = false, $widget_options = array(), $control_options = array() ) {
		...
		$name    = ( $name ) ? $name : __( 'Image', 'wp-image-widget' );

		$widget_options = wp_parse_args(
			$widget_options,
			array(
				'classname'   => 'widget_simpleimage', // Legacy class name.
				'description' => __( 'An image from your Media Library.', 'wp-image-widget' ),
			)
		);
	...
	public function render( $args, $instance ) {
		...
		if ( $inside ) {
			...
			$template_loader = new WP_Image_Widget_Template_Loader();
	...
	public function form( $instance ) {
		...
		$button_class = array( 'button', 'button-hero', 'wp-image-widget-control-choose' );
		...
		<div class="wp-image-widget-form">
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'wp-image-widget' ); ?></label>
			...
			<?php if ( ! is_simple_image_widget_legacy() ) : ?>
				<p class="wp-image-widget-control<?php echo ( $image_id ) ? ' has-image' : ''; ?>"
					data-title="<?php esc_attr_e( 'Choose an Image', 'wp-image-widget' ); ?>"
					data-update-text="<?php esc_attr_e( 'Update Image', 'wp-image-widget' ); ?>"
					...
					?>
					<input type="hidden" name="<?php echo esc_attr( $this->get_field_name( 'image_id' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'image_id' ) ); ?>" value="<?php echo absint( $image_id ); ?>" class="image-id wp-image-widget-control-target">
					<a href="#" class="<?php echo esc_attr( join( ' ', $button_class ) ); ?>"><?php _e( 'Choose an Image', 'wp-image-widget' ); ?></a>
				</p>
			<?php endif; ?>
					...
					switch ( $field ) {
						case 'image_size' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'image_size' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php _e( 'Size:', 'wp-image-widget' ); ?></label>
						...
						case 'link' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php _e( 'Link:', 'wp-image-widget' ); ?></label>
								<span class="wp-image-widget-input-group">
									<input type="text" name="<?php echo esc_attr( $this->get_field_name( 'link' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>" value="<?php echo esc_url( $instance['link'] ); ?>" class="wp-image-widget-input-group-field">
									<button class="wp-image-widget-find-posts-button wp-image-widget-input-group-button dashicons dashicons-search"></button>
								</span>
							</p>
							<p class="<?php echo esc_attr( $this->siw_field_class( 'new_window' ) ); ?>" style="margin-top: -0.75em; padding-left: 2px">
								<label for="<?php echo esc_attr( $this->get_field_id( 'new_window' ) ); ?>">
									...
									<?php _e( 'Open in new window?', 'wp-image-widget' ); ?>
								</label>
							</p>
							<?php
							break;
						...
						case 'link_classes' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_classes' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_classes' ) ); ?>"><?php _e( 'Link Classes:', 'wp-image-widget' ); ?></label>
						...
						case 'link_text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_text' ) ); ?>"><?php _e( 'Link Text:', 'wp-image-widget' ); ?></label>
						...
						case 'text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php _e( 'Text:', 'wp-image-widget' ); ?></label>
	...
	public function get_image_sizes( $image_id ) {
		$sizes = array( 'full' => __( 'Full Size', 'wp-image-widget' ) );

		$imagedata = wp_get_attachment_metadata( $image_id );
		if ( isset( $imagedata['sizes'] ) ) {
			$size_names = WP_Image_Widget_Plugin::get_image_size_names();
	...
	protected function siw_field_class( $id ) {
		$classes = array( 'wp-image-widget-field', 'wp-image-widget-field-' . sanitize_html_class( $id ) );

		$hidden_fields = WP_Image_Widget_Plugin::get_hidden_fields();
