 ...
 *
 * @package WPImageWidget
 * @since 4.0.0
 */
class WP_Image_Widget_Legacy {
	/**
	 * Load legacy support.
	 *
	 * @since 4.0.0
	...
	public function display_fields( $instance, $widget ) {
		if ( is_simple_image_widget_legacy() || ! empty( $instance['image'] ) ) :
			?>
			<div class="wp-image-widget-legacy-fields">
				<?php if ( ! is_simple_image_widget_legacy() ) : ?>
					<p>
						<em><?php _e( 'These fields are here to maintain your data from an earlier version.', 'wp-image-widget' ); ?></em>
					</p>
					<p>
						<em><?php _e( 'Select an image, then clear these values, and they will disappear when you save the widget.', 'wp-image-widget' ); ?></em>
					</p>
				<?php endif; ?>

				<p>
					<label for="<?php echo esc_attr( $widget->get_field_id( 'image' ) ); ?>"><?php _e( 'Image URL:', 'wp-image-widget' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $widget->get_field_id( 'alt' ) ); ?>"><?php _e( 'Alternate Text:', 'wp-image-widget' ); ?></label>
	 ...
	 *
	 * Called in WP_Image_Widget::update().
	 *
	 * @since 4.0.0
	 *
