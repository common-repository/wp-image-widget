...
class WP_Image_Widget_Plugin {
	...
	public function load() {
		...
		$compat = new WP_Image_Widget_Legacy();
		$compat->load();
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'wp-image-widget', false, dirname( dirname( plugin_basename( __FILE__ ) ) ) . '/languages' );
	}
	...
	public function register_widget() {
		register_widget( 'WP_Image_Widget' );
	}
	 ...
	 * 'wp-image-widget-control' script a bit in order to allow it to be
	...
	public function register_assets() {
		wp_register_style(
			'wp-image-widget-admin',
			dirname( plugin_dir_url( __FILE__ ) ) . '/assets/css/wp-image-widget.css'
		);

		wp_register_script(
			'wp-image-widget-admin',
			dirname( plugin_dir_url( __FILE__ ) ) . '/assets/js/wp-image-widget.js',
		...
		wp_localize_script(
			...
			array(
				'l10n' => array(
					'frameTitle'      => __( 'Choose an Attachment', 'wp-image-widget' ),
					'frameUpdateText' => __( 'Update Attachment', 'wp-image-widget' ),
					'fullSizeLabel'   => __( 'Full Size', 'wp-image-widget' ),
					'imageSizeNames'  => self::get_image_size_names(),
					'responseError'   => __( 'An error has occurred. Please reload the page and try again.', 'wp-image-widget' ),
				),
	...
	public function widgets_screen_settings( $settings, $screen ) {
		...
		$settings .= sprintf( '<h5>%s</h5>', __( 'WP  Image Widget', 'wp-image-widget' ) );

		$fields = array(
			'image_size'   => __( 'Image Size', 'wp-image-widget' ),
			'link'         => __( 'Link', 'wp-image-widget' ),
			'link_classes' => __( 'Link Classes', 'wp-image-widget' ),
			'link_text'    => __( 'Link Text', 'wp-image-widget' ),
			'new_window'   => __( 'New Window', 'wp-image-widget' ),
			'text'         => __( 'Text', 'wp-image-widget' ),
		);
		...
		foreach ( $fields as $id => $label ) {
			$settings .= sprintf(
				'<label><input type="checkbox" value="%1$s"%2$s class="wp-image-widget-field-toggle"> %3$s</label>',
				esc_attr( $id ),
	...
	public function enqueue_admin_assets() {
		wp_enqueue_media();
		wp_enqueue_script( 'wp-image-widget-admin' );
		wp_enqueue_script( 'wp-image-widget-find-posts' );
		wp_enqueue_style( 'wp-image-widget-admin' );
	}
	...
	public static function get_image_size_names() {
		return apply_filters(
			...
			array(
				'thumbnail' => __( 'Thumbnail', 'wp-image-widget' ),
				'medium'    => __( 'Medium', 'wp-image-widget' ),
				'large'     => __( 'Large', 'wp-image-widget' ),
				'full'      => __( 'Full Size', 'wp-image-widget' ),
			)
		);
	}
	...
	public function ajax_find_posts() {
		...
		if ( ! $posts ) {
			wp_send_json_error( __( 'No items found.', 'wp-image-widget' ) );
		}
	...
	protected function get_found_posts_html( $posts ) {
		$html = sprintf(
			...
			__( 'Title', 'wp-image-widget' ),
			__( 'Type', 'wp-image-widget' ),
			__( 'Date', 'wp-image-widget' ),
			__( 'Status', 'wp-image-widget' )
		);
		...
		foreach ( $posts as $post ) {
			...
			switch ( $post->post_status ) {
				case 'publish' :
				case 'private' :
					$status = __( 'Published', 'wp-image-widget' );
					break;
				case 'future' :
					$status = __( 'Scheduled', 'wp-image-widget' );
					break;
				case 'pending' :
					$status = __( 'Pending Review', 'wp-image-widget' );
					break;
				case 'draft' :
					$status = __( 'Draft', 'wp-image-widget' );
					break;
			}
	...
	public function print_find_posts_templates() {
		?>
		<script type="text/html" id="tmpl-wp-image-widget-modal">
			<div class="wp-image-widget-modal-head find-box-head">
				<?php _e( 'Find Post', 'wp-image-widget' ); ?>
				<div class="wp-image-widget-modal-close js-close"></div>
			</div>
			<div class="wp-image-widget-modal-inside find-box-inside">
				<div class="wp-image-widget-modal-search find-box-search">
					<?php wp_nonce_field( 'siw-find-posts', 'siw-find-posts-ajax-nonce', false ); ?>
					<input type="text" name="s" value="" class="wp-image-widget-modal-search-field">
					<span class="spinner"></span>
					<input type="button" value="<?php esc_attr_e( 'Search', 'wp-image-widget' ); ?>" class="button wp-image-widget-modal-search-button" />
					<div class="clear"></div>
				</div>
				<div class="wp-image-widget-modal-response"></div>
			</div>
		</script>
		<?php
	}
